...
# Tabela pomocnicza: Użytkownicy <-> Grupy (Many-to-Many)
...
# Tabela pomocnicza: Media <-> Kategorie (Many-to-Many)
media_kategorie = db.Table(
    "media_kategorie",
    db.Column("media_id", db.Integer, db.ForeignKey("media.id"), primary_key=True),
    db.Column(
        "kategoria_id", db.Integer, db.ForeignKey("kategorie.id"), primary_key=True
    ),
)
