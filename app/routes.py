    ...
    if current_user.is_authenticated:
        ...
        if current_user.grupy:
            ...
            if len(aktywna_grupa.czlonkowie) > 1:
                partner = [u for u in aktywna_grupa.czlonkowie if u.id != current_user.id][0]
                match_data = get_match_between_users(current_user.id, partner.id, aktywna_grupa.id)
...
def register():
    if request.method == 'POST':
        ...
        istnieje = Uzytkownik.query.filter((Uzytkownik.email == email) | (Uzytkownik.nazwa_uzytkownika == nazwa_uzytkownika)).first()
        if istnieje:
...
@login_required
def rate():
    # Wybieramy film, którego użytkownik jeszcze nie ocenił
    ocenione_ids = [g.media_id for g in current_user.glosy]
    film = Media.query.filter(~Media.id.in_(ocenione_ids)).order_by(func.random()).first()
...
def rate_submit():
    ...
    # Dodajemy ocenę do wszystkich grup użytkownika, aby match się aktualizował
    for grupa in current_user.grupy:
        istniejacy_glos = Glos.query.filter_by(uzytkownik_id=current_user.id, media_id=media_id, grupa_id=grupa.id).first()
        if istniejacy_glos:
